...
const blankProj = (): Project => ({ name: "", description: "", link: "", id: genId() });
...
const BuildResumePage = () => {
  ...
  function updateExp(id: string, key: keyof Experience, val: any) {
    setExp((p) => p.map((e) => (e.id === id ? { ...e, [key]: val } : e)));
  }

  function updateButtet(eid: string, bi: number, val: string) {
    setExp((p) =>
      p.map((e) =>
        e.id === eid ? { ...e, bullets: e.bullets.map((b, j) => (j === bi ? val : b)) } : e
      )
    );
  }
                ...
                >
                  <div className="flex justify-between items-center">
                    ...
                    {experience.length > 1 && (
                      <button
                        onClick={() =>
                          setExp((p) => p.filter((it) => it.id !== exp.id))
                        }
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <Field
                      label="Job Title"
                      value={exp.title}
                      onChange={(v: string) => updateExp(exp.id!, "title", v)}
                      placeholder="Software Engineer"
                    />
                    <Field
                      label="Company"
                      value={exp.company}
                      onChange={(v: string) => updateExp(exp.id!, "company", v)}
                      placeholder="Google"
                    />
                    <Field
                      label="Location"
                      value={exp.location}
                      onChange={(v: string) => updateExp(exp.id!, "location", v)}
                      placeholder="Ranchi, jharkhand"
                    />
                    <Field
                      label="Start Date"
                      value={exp.startDate}
                      onChange={(v: string) => updateExp(exp.id!, "startDate", v)}
                      placeholder="April 2026"
                    />
                    <Field
                      label="End Date"
                      value={exp.endDate}
                      onChange={(v: string) => updateExp(exp.id!, "endDate", v)}
                      placeholder="April 2026"
                    />
                  </div>
                  <div className="flex flex-col gap-2">
                    ...
                    {exp.bullets.map((b, bi) => (
                      <div key={bi} className="flex gap-2">
                        <input
                          value={b}
                          onChange={(e) => updateButtet(exp.id!, bi, e.target.value)}
                          placeholder={`Bullet ${
                            bi + 1
                ...
                >
                  <div className="flex justify-between items-center">
                    ...
                    {education.length > 1 && (
                      <button
                        onClick={() =>
                          setEdu((p) => p.filter((it) => it.id !== edu.id))
                        }
                        className="text-red-400/60 hover:text-red-400 transition-colors"
                      >
                        <Trash size={14} />
                ...
                >
                  <div className="flex justify-between items-center">
                    ...
                    {projects.length > 1 && (
                      <button
                        onClick={() =>
                          setProjects((p) => p.filter((it) => it.id !== proj.id))
                        }
                        className="text-red-400/60 hover:text-red-400 transition-colors"
                      >
                        <Trash size={14} />
